...
{
    //飞行模式判断选择
    if (!((rc_data.channels[6] > rc_data_channel6_last - 20) && (rc_data.channels[6] < rc_data_channel6_last + 20)))
    ...
    //解锁 arm判断
    // ROS_ERROR("channel5 value:%d",rc_data.channels[5]);
...
地面站期望控制数据接收
*/
// void controldataCallback(const quadrotor_msgs::PositionCommand::ConstPtr& msg)
// {
...
//     ROS_INFO("pos = [%.3f, %.3f, %.3f]", msg->position.x, msg->position.y, msg->position.z);
// }
...
int px4control_init(ros::NodeHandle nh)
{
    ...
    // control_data_sub = nh.subscribe<quadrotor_msgs::PositionCommand>("/position_cmd",2,controldataCallback);
    straight_control_data_sub = nh.subscribe<geometry_msgs::PoseStamped>("straight_output_target",2,straightControldataCallback);
    // ego_control_data_sub = nh.subscribe<geometry_msgs::PoseStamped>("ego_input_target",2,EGOControldataCallback);
